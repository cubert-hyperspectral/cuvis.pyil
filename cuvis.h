 ...
 *
 * Replaces a white reference measurement for reflectance calculation. The counts are
 * interpreted against i_effectiveBitDepth (1 to 16); i_integrationTime [ms] and
 * i_loadLevel describe the recording conditions. Wavelengths are nanometres.
 * Slot: Reference_WhiteSpectrum. */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_set_reference_white_spectrum(
    CUVIS_PROC_CONT i_procCont, float const* i_wavelengths, uint16_t const* i_values, uint32_t i_count, uint16_t i_effectiveBitDepth, double i_integrationTime);

/** @brief Set the white-target reflectance spectrum, values in percent (0 to 100).
 *
 * Wavelengths are nanometres. Slot: Reference_TargetSpectrum. */
SDK_CAPI CUVIS_STATUS SDK_CCALL
    cuvis_proc_cont_set_reference_target_spectrum(CUVIS_PROC_CONT i_procCont, float const* i_wavelengths, float const* i_values, uint32_t i_count);

/** @brief Number of samples of the spectrum in the given slot
 * (Reference_WhiteSpectrum or Reference_TargetSpectrum).
 * Returns status_not_available when the slot is empty. */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_get_reference_spectrum_size(CUVIS_PROC_CONT i_procCont, CUVIS_REFERENCE_TYPE i_type, uint32_t* o_pCount);

/** @brief Copy out the white reference spectrum (Reference_WhiteSpectrum).
 *
 * Both buffers must hold at least the count reported by
 * cuvis_proc_cont_get_reference_spectrum_size. Wavelengths in nanometres. */
SDK_CAPI CUVIS_STATUS SDK_CCALL
    cuvis_proc_cont_get_reference_white_spectrum(CUVIS_PROC_CONT i_procCont, float* o_pWavelengths, uint16_t* o_pValues, uint32_t i_capacity);

/** @brief Copy out the target reflectance spectrum (Reference_TargetSpectrum), percent. */
SDK_CAPI CUVIS_STATUS SDK_CCALL
    cuvis_proc_cont_get_reference_target_spectrum(CUVIS_PROC_CONT i_procCont, float* o_pWavelengths, float* o_pValues, uint32_t i_capacity);
